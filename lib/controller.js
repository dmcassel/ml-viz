...
//      };
      ...
      for (facet in query.facets) {
        if (query.facets.hasOwnProperty(facet)) {
          ...
          if (query.facets[facet].negate === true) {
            negatedQuery = facetQ;
            facetQ = {};
            facetQ["not-query"] = {};
            facetQ["not-query"]["queries"] = [];
            facetQ["not-query"]["queries"].push(negatedQuery);
          }
    ...
    updateQuery = function (queryUpdate) {
      ...
      if (queryUpdate.facet) {
        ...
        if (queryUpdate.negate) {
          newQuery.facets[queryUpdate.facet].negate = queryUpdate.negate;
        } else {
          newQuery.facets[queryUpdate.facet].negate = false;
        }
    ...
    getBox = (function () {
      ...
      return function (queryUpdate, target) {
        ...
        for (facet in oldQuery.facets) {
          if (oldQuery.facets.hasOwnProperty(facet)) {
            facetQ = {};
  ...
  var bookmark = (function () {
    var facetSeparator = '*__*';
    var facetDelimiter = '*_*';
    var negateDelimiter = '*!*';
    ...
    var hashQuery = function (query) {
        ...
        for (facet in query.facets) {
          if (query.facets.hasOwnProperty(facet)) {
            if (!(query.facets[facet].geo || query.facets[facet].constraintType === 'geo')) {
              if (query.facets[facet].negate === true) {
                facets += facet + negateDelimiter + query.facets[facet].value[0] + facetSeparator;
              } else {
                facets += facet + facetDelimiter + query.facets[facet].value[0] + facetSeparator;
              }
            }
          }
        }
      ...
      if (facets) {
        bookmarkQuery.f = facets.substr(0, facets.length - facetSeparator.length);
      }
    ...
    var getUrlQuery = function () {
      var param,
        ...
        facet = '',
        facets,
        // Allow for *_* (positive) or *!* (negative)
        delimPattern = /\*_\*|\*!\*/,
        negatePattern = /\*!\*/,
        rawFacet;
      ...
      for (i = 0; i < l; i = i + 1) {
        ...
        if (param[0] === 'q') {
          ...
          facets = param[1].split(facetSeparator);
          m = facets.length;
          query.facets = {};

          for (j = 0; j < m; j = j + 1) {
            rawFacet = decodeURIComponent(facets[j]);
            facet = rawFacet.split(delimPattern);
            query.facets[facet[0]] = {};
            query.facets[facet[0]].value = [facet[1].replace(/\+/g, ' ')];
            query.facets[facet[0]].constraintType = facetTypes[facet[0]];
            query.facets[facet[0]].negate = rawFacet.match(negatePattern) !== null;
          }
        }
      }
