...
ML.createResults = function (containerID, config) {
  ...
  truncStr = function (str, len) {
    var trunc, truncArr;
    if (str.length > len) {
      trunc = str.substr(0, len);
      truncArr = trunc.split(" ");
  ...
  renderCB = function (data) {
    var s, i, j, k, c, key, matches, shortSnippetUpdated;
    if (!data.boundedbox) {
      /******************/
      /* Results header */
      /******************/
      ...
      if (config.metadata.sort && !$.isEmptyObject(config.metadata.sort)) {
        ...
        for (s in config.metadata.sort) {
          if (config.metadata.sort.hasOwnProperty(s)) {
      ...
      for (i = 0; i < data['results'].length; i++) {
        ...
        try {
          ...
          for (c in abstractTitle) {
            if (abstractTitle.hasOwnProperty(c)) {
              // literal is user-defined text
              if (abstractTitle[c]['type'] === 'literal') {
                titleContent += abstractTitle[c]['text'];
          ...
          for (k in matches) {
            if (matches.hasOwnProperty(k)) {
              ...
              if (data['results'][i].matches[k]['match-text'] instanceof Array) {
                ...
                // this matching text
                shortSnippetUpdated = false;
                // concat array terms
                for (j in data['results'][i].matches[k]['match-text']) {
                  if (data['results'][i].matches[k]['match-text'].hasOwnProperty(j)) {
                    // mark up the highlighted terms

                    if (data['results'][i].matches[k]['match-text'][j] instanceof Object) {
                      longSnippet += '<span class="highlight">'
                        + data['results'][i].matches[k]['match-text'][j]['highlight']
                        + '</span>';
                      ...
                      longSnippet += data['results'][i].matches[k]['match-text'][j] + ' ';
              ...
              // otherwise, snippet is a string
                ...
                longSnippet += data['results'][i].matches[k]['match-text'];
        ...
        try {
          ...
          for (c in abstractMetadata) {
            if (abstractMetadata.hasOwnProperty(c)) {
              // literal is user-defined text
              if (abstractMetadata[c]['type'] === 'literal') {
                metaContent += abstractMetadata[c]['text'];
              // value is from the document XML
